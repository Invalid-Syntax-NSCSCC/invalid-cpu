...
import pipeline.dispatch.{CsrScoreboard, IssueStage, RegReadStage, Scoreboard}
...
class CoreCpuTop extends Module {
  ...
  val csrScoreBoard = Module(new CsrScoreboard(changeNum = Param.csrScoreBoardChangeNum, regNum = Count.csr))
